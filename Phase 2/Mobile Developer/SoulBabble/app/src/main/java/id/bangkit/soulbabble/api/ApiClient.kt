...
class ApiClient {
    ...
    private val baseUrl = "https://b74bbf98-e820-4175-8369-6b07a1d32ab7.mock.pstmn.io/"  // Ganti dengan URL server Anda
    ...
    fun getDetailRecommended(
        apiKey: String,
        token: String,
        journalId: String,
        callback: (result: String?, error: String?) -> Unit
    ) {
        val url = "${baseUrl}getDetailRecommedation"
        val requestBody = FormBody.Builder()
            .add("id", journalId)
            .build()
    ...
    suspend fun getNotificationData(
        ...
        return withContext(Dispatchers.IO) {
            // Panggilan HTTP (contoh menggunakan OkHttp)
            try {
